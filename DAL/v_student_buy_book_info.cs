...
{
	/// <summary>
	/// 数据访问类:v_student_buy_book_info
	/// </summary>
	public partial class v_student_buy_book_info
	{
		public v_student_buy_book_info()
		{}
		#region  BasicMethod
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(TSIMSServer.Model.v_student_buy_book_info model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into v_student_buy_book_info(");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(TSIMSServer.Model.v_student_buy_book_info model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update v_student_buy_book_info set ");
			...
			MySqlParameter[] parameters = {
					...
					new MySqlParameter("@publisher", MySqlDbType.VarChar,100),
					new MySqlParameter("@edition", MySqlDbType.VarChar,100),
					new MySqlParameter("@amount", MySqlDbType.Date,21)};
			parameters[0].Value = model.college_num;
			parameters[1].Value = model.college_name;
			parameters[2].Value = model.department_num;
			parameters[3].Value = model.department_name;
		...
		public bool Delete()
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from v_student_buy_book_info ");
			strSql.Append(" where ");
			MySqlParameter[] parameters = {
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public TSIMSServer.Model.v_student_buy_book_info GetModel()
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select college_num,college_name,department_num,department_name,major_num,major_name,class_num,class_name,semester,course_num,course_name,course_type,need_book,teacher_num,teacher_name,isbn,book_name,author,price,publisher,edition,amount from v_student_buy_book_info ");
			strSql.Append(" where ");
			MySqlParameter[] parameters = {
			};

			TSIMSServer.Model.v_student_buy_book_info model=new TSIMSServer.Model.v_student_buy_book_info();
		...
		/// 得到一个对象实体
		/// </summary>
		public TSIMSServer.Model.v_student_buy_book_info DataRowToModel(DataRow row)
		{
			TSIMSServer.Model.v_student_buy_book_info model=new TSIMSServer.Model.v_student_buy_book_info();
			if (row != null)
			{
		...
		public DataSet GetList(string strWhere)
		{
			...
			strSql.Append(" FROM v_student_buy_book_info ");
			if(strWhere.Trim()!="")
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM v_student_buy_book_info ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from v_student_buy_book_info T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "v_student_buy_book_info";
